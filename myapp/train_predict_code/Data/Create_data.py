...
def Createdata(graph, files):  # 数据库名称, 文件路径/文件名, 数据来源[1 召回 2 投诉 3 实际发生新闻]
        ...
        while 1:
            ...
            if line:
                if counts == eventnum + 1:
                    ...
                    if JudgeDataExist(graph, p[date_index], p[title_index]):
...
if __name__ == '__main__':
    ...
    getrel(graph)
    print(GetEventNum(graph))
    # graph.delete_all()  # 清除neo4j中原有的结点等所有信息
    Createdata(graph, '标准院-国内召回情况-2021年6月有效数据.tsv')  # 数据库名称, 文件路径/文件名, 数据来源[1 召回 2 投诉 3 实际发生新闻]
    Createdata(graph, '标准院-国外召回情况-2021年6月-有效数据.tsv')  # 数据库名称, 文件路径/文件名, 数据来源[1 召回 2 投诉 3 实际发生新闻]
    Createdata(graph, '消费品召回1.tsv')  # 数据库名称, 文件路径/文件名, 数据来源[1 召回 2 投诉 3 实际发生新闻]
    Createdata(graph, '消费品质量安全事件统计2020年5月-6月-有效数据.tsv')  # 数据库名称, 文件路径/文件名, 数据来源[1 召回 2 投诉 3 实际发生新闻]
