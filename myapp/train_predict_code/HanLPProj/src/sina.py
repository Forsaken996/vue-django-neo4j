...
class SinaTousu(object):
    ...
    def __init__(self, time_:str, type_:int):
        super().__init__()
...
#             os.remove("pause.conf")
    ...
    def storeData(self):
        ...
        if not self.data:
            return
        with open("./data.csv", "a+", encoding=self.encode, newline="") as f:
            # print(1)
            f_csv = csv.writer(f)
            f_csv.writerow(self.data[0].keys())
            for i in self.data:
                # print(i)
                try:
...
if __name__ == "__main__":
    b = datelist("2021-03-26","2021-04-25")
    a = SinaTousu(b,0)
    a.run()
